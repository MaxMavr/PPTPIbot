...
from utils.links import make_yandex_song_link, parse_yandex_music_link
from utils.music_yandex import get_admin_song, get_song_artist_title_by_song_id
...
async def _(message: Message, yandex_link):
    ...
    await message.answer(
        PHRASES_RU.replace('success.save_admin_song', song=msg_song_text),
...
async def _(message: Message, yandex_link):
    ...
    await message.answer(
        PHRASES_RU.replace('success.save_mood_song', song=msg_song_text),
        disable_web_page_preview=True,
    )
